...
- **尾行** —— 待办完成数（`✓3/5`）、模型名、本次花费、累计时长。待办优先取当前活动子代理（Task）的列表：子代理的待办写在它自己独立的日志里（`<主会话id>/subagents/agent-*.jsonl`），主会话日志里没有；子代理运行期间父会话日志静止，故「比主日志更新的子代理日志」即活动来源，否则回落主会话自己的待办。
